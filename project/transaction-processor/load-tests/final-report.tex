...
\begin{lstlisting}[caption={}, label={}, basicstyle=\ttfamily\small, frame=none]
  ...
  Application Load Balancer
        |
        v
  ECS Fargate: api-svc  ----------->  DynamoDB: transactions (PENDING)
        |
        | SendMessage
        v
  Amazon SQS (transfers queue) + DLQ
        |
        | ReceiveMessage
        v
  ECS Fargate: worker-svc
   |           |            |
   v           v            v
 DynamoDB   DynamoDB     CloudWatch Logs
 accounts   account_locks  (app + METRICS snapshots)
 (TransactWriteItems: debit + credit + mark COMPLETED)
\end{lstlisting}
\caption{Deployed architecture. Every arrow crosses an AWS service boundary; no component runs locally.}
\label{fig:arch}
\end{figure}
